...
import ai.openpaw.android.ScreenCaptureRequester
...
class ScreenRecordManager(private val context: Context) {
  ...
  @Volatile private var permissionRequester: ai.openpaw.android.PermissionRequester? = null

  fun attachScreenCaptureRequester(requester: ScreenCaptureRequester) {
    screenCaptureRequester = requester
  }

  fun attachPermissionRequester(requester: ai.openpaw.android.PermissionRequester) {
  ...
  suspend fun record(paramsJson: String?): Payload =
    withContext(Dispatchers.Default) {
      ...
      val file = File.createTempFile("openpaw-screen-", ".mp4")
