...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.openpaw", category: "gateway-endpoint")
    ...
    {
        let raw = env["OPENPAW_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
               ...
               !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "OPENPAW_GATEWAY_PASSWORD",
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["OPENPAW_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
    ...
    private let logger = Logger(subsystem: "ai.openpaw", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "openpaw.onboardingSeen")
    ...
    {
        if let envBind = env["OPENPAW_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["OPENPAW_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
