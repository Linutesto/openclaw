...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.openpaw/openpaw.json. Prefer OPENPAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.openpaw/openpaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.openpaw/openpaw.json. Restart the gateway to apply."
        }
