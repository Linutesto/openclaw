...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENPAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/openpaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openpaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openpaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openpaw/ (rolling or stdout), or nil if none exist.
