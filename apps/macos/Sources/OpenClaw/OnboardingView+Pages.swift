...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 22) {
                Text("Welcome to OpenPaw")
                    .font(.largeTitle.weight(.semibold))
                Text("OpenPaw is a powerful personal AI assistant that can connect to WhatsApp or Telegram.")
                    .font(.body)
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(
                                "The connected AI agent (e.g. Claude) can trigger powerful actions on your Mac, " +
                                    ...
                                    "Only enable OpenPaw if you understand the risks and trust the prompts and " +
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("Choose your Gateway")
                .font(.largeTitle.weight(.semibold))
            Text(
                "OpenPaw uses a single Gateway that stays running. Pick this Mac, " +
                                ...
                                if self.state.remoteTransport == .ssh {
                                    ...
                                    GridRow {
                                        Text("CLI path")
                                            ...
                                            .frame(width: labelWidth, alignment: .leading)
                                        TextField(
                                            "/Applications/OpenPaw.app/.../openpaw",
    ...
    func anthropicAuthPage() -> some View {
        self.onboardingPage {
            ...
            Text("Give your model the token it needs!")
                ...
                .frame(maxWidth: 540)
                .fixedSize(horizontal: false, vertical: true)
            Text("OpenPaw supports any model — we strongly recommend Opus 4.5 for the best experience.")
                .font(.callout)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 540)
            ...
            self.onboardingCard(spacing: 12, padding: 16) {
                ...
                Text(
                    "This lets OpenPaw use Claude immediately. Credentials are stored at " +
                        "`~/.openpaw/credentials/oauth.json` (owner-only).")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    func permissionsPage() -> some View {
        self.onboardingPage {
            Text("Grant permissions")
                .font(.largeTitle.weight(.semibold))
            Text("These macOS permissions let OpenPaw automate apps and capture context on this Mac.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func cliPage() -> some View {
        self.onboardingPage {
            Text("Install the CLI")
                .font(.largeTitle.weight(.semibold))
            Text("Required for local mode: installs `openpaw` so launchd can run the gateway.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("Agent workspace")
                .font(.largeTitle.weight(.semibold))
            Text(
                "OpenPaw runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.openpaw/openpaw.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `openpaw` package and make sure credentials exist
                        (typically `~/.openpaw/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the OpenPaw menu bar icon for quick chat and status.",
