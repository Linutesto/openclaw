...
final class ScreenRecordService {
    ...
    private let logger = Logger(subsystem: "ai.openpaw", category: "screenRecord")
        ...
        outPath: String?) async throws -> (path: String, hasAudio: Bool)
    {
        ...
        let outURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("openpaw-screen-record-\(UUID().uuidString).mp4")
        }()
...
private final class StreamRecorder: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    let queue = DispatchQueue(label: "ai.openpaw.screenRecord.writer")
