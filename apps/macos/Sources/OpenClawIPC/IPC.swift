...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/OpenPaw/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    let preferred = home
        .appendingPathComponent("Library/Application Support/OpenPaw/control.sock")
        .path
    return preferred
}()
