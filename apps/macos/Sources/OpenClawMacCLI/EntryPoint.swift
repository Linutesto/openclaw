...
struct OpenClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("openpaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      openpaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      openpaw-mac discover [--timeout <ms>] [--json] [--include-local]
      openpaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      openpaw-mac connect
      openpaw-mac connect --url ws://127.0.0.1:18789 --json
      openpaw-mac discover --timeout 3000 --json
      openpaw-mac wizard --mode local
    """)
}
