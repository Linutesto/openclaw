...
- Jobs persist under `~/.openpaw/cron/` so restarts don’t lose schedules.
...
openpaw cron list
openpaw cron run <job-id> --force
openpaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openpaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openpaw cron add/edit` or the cron
...
- Job store: `~/.openpaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openpaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
openpaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openpaw cron edit <jobId> --agent ops
openpaw cron edit <jobId> --clear-agent
```
...
```bash
openpaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openpaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENPAW_SKIP_CRON`.
