...
Deploy OpenPaw on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy OpenPaw](https://northflank.com/stacks/deploy-openpaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy OpenPaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the OpenPaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the OpenPaw service.
8. Open the public OpenPaw URL and complete setup at `/setup`.
9. Open the Control UI at `/openpaw`.

## What you get

- Hosted OpenPaw Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/openpaw`
