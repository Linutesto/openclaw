...
Matrix is an open, decentralized messaging protocol. OpenPaw connects as a Matrix **user**
...
```bash
openpaw plugins install @openpaw/matrix
```

Local checkout (when running from a git repo):

```bash
openpaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openpaw plugins install @openpaw/matrix`
   - From a local checkout: `openpaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenPaw calls the same
     login endpoint, stores the access token in `~/.openpaw/credentials/matrix/credentials.json`,
...
- On first connection, OpenPaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenPaw logs a warning.
...
`~/.openpaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openpaw pairing list matrix`
  - `openpaw pairing approve matrix <CODE>`
...
- On startup, OpenPaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
