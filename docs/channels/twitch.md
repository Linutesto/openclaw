...
read_when:
  - Setting up Twitch chat integration for OpenPaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenPaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openpaw plugins install @openpaw/twitch
```

Local checkout (when running from a git repo):

```bash
openpaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENPAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openpaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENPAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENPAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
