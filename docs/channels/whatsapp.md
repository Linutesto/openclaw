...
2. Configure WhatsApp in `~/.openpaw/openpaw.json`.
3. Run `openpaw channels login` to scan the QR code (Linked Devices).
4. Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run OpenPaw on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for OpenPaw. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the OpenPaw number there.
...
Quick fallback: run OpenPaw on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[openpaw]`)
...
- Early OpenPaw builds supported Twilio’s WhatsApp Business integration.
...
- Login command: `openpaw channels login` (QR via Linked Devices).
- Multi-account login: `openpaw channels login --account <id>` (`<id>` = `accountId`).
- Default account (when `--account` is omitted): `default` if present, otherwise the first configured account id (sorted).
- Credentials stored in `~/.openpaw/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.openpaw/credentials/`.
- Logout: `openpaw channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `openpaw pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run OpenPaw on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will OpenPaw message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. OpenPaw only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:

- First DM from a new sender returns a short code (message is not processed).
- Approve with: `openpaw pairing approve whatsapp <code>` (list with `openpaw pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different OpenPaw instances on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `openpaw message send --media <mp4> --gif-playback`
...
- Best results: OGG/Opus. OpenPaw rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/openpaw/openpaw-YYYY-MM-DD.log` (configurable).
...
- Fix: run `openpaw channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**

- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `openpaw doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `openpaw logs --follow`.
