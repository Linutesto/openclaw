...
3. Set tokens for OpenPaw and start the gateway.
...
7. Slash Commands → create `/openpaw` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
OpenPaw can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, OpenPaw falls back to the user token.
...
```json
{
  "display_information": {
    "name": "OpenPaw",
    "description": "Slack connector for OpenPaw"
  },
  "features": {
    "bot_user": {
      "display_name": "OpenPaw",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openpaw",
        "description": "Send a message to OpenPaw",
        "should_escape": false
      }
    ]
  },
...
By default, OpenPaw replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, OpenPaw infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `openpaw pairing approve slack <code>`.
...
- On startup, OpenPaw resolves channel/user names in allowlists to IDs (when tokens allow)
