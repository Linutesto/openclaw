...
read_when:
  - You want OpenPaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenPaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`openpaw onboard`) and `openpaw channels add` list optional channel plugins.
...
```bash
openpaw plugins install @openpaw/nostr
```

Use a local checkout (dev workflows):

```bash
openpaw plugins install --link <path-to-openpaw>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "openpaw@example.com",
        "lud16": "openpaw@example.com"
      }
    }
  }
}
