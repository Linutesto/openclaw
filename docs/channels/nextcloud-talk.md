...
```bash
openpaw plugins install @openpaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openpaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "OpenPaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openpaw pairing list nextcloud-talk`
  - `openpaw pairing approve nextcloud-talk <CODE>`
