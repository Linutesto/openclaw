...
- Install via CLI: `openpaw plugins install @openpaw/zalo`
...
1. Install the Zalo plugin:
   - From a source checkout: `openpaw plugins install ./extensions/zalo`
   - From npm (if published): `openpaw plugins install @openpaw/zalo`
...
- Approve via:
  - `openpaw pairing list zalo`
  - `openpaw pairing approve zalo <CODE>`
...
- Example: `openpaw message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `openpaw channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `openpaw logs --follow`
