...
3. Configure OpenPaw with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for OpenPaw + `imsg` (Messages DB access).
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. OpenPaw only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. OpenPaw can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, OpenPaw attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - openpaw gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.openpaw/scripts/imsg-ssh",
...
Example wrapper (`~/.openpaw/scripts/imsg-ssh`):
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.openpaw/openpaw.json` (it often contains tokens).
...
- Approve via:
  - `openpaw pairing list imessage`
  - `openpaw pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, OpenPaw treats that thread as a “group” for:
