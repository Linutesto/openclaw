...
Tlon is a decentralized messenger built on Urbit. OpenPaw connects to your Urbit ship and can
...
```bash
openpaw plugins install @openpaw/tlon
```

Local checkout (when running from a git repo):

```bash
openpaw plugins install ./extensions/tlon
```
...
Use these with `openpaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenPaw replies in-thread.
