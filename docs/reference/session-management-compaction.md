...
This document explains how OpenPaw manages sessions end-to-end:
...
OpenPaw is designed around a single **Gateway process** that owns session state.
...
OpenPaw persists sessions in two layers:
...
- Transcripts: `~/.openpaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

OpenPaw resolves these via `src/config/sessions.ts`.
...
OpenPaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OpenPaw consumes the events, but Pi decides when to compact).
...
OpenPaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OpenPaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OpenPaw leaves it alone.
...
- `openpaw status` (CLI)
- `openpaw sessions` / `sessions --json`
...
OpenPaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OpenPaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, OpenPaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OpenPaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OpenPaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `openpaw status`.
