...
OpenPaw features that can generate provider usage or paid API calls.
...
- `openpaw status --usage` and `openpaw channels list` show provider **usage windows**
...
OpenPaw can pick up credentials from:
...
- `openpaw status --usage`
- `openpaw models status --json`
...
`openpaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
