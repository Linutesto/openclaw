...
Script: [`scripts/bench-model.ts`](https://github.com/openpaw/openpaw/blob/main/scripts/bench-model.ts)
...
This script drives the interactive wizard via a pseudo-tty, verifies config/workspace/session files, then starts the gateway and runs `openpaw health`.
