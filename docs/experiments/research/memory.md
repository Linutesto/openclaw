...
read_when:
  - Designing workspace memory (~/.openpaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenPaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openpaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openpaw/workspace` as canonical human-readable memory.
...
```
~/.openpaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenPaw**, but keep a separable core library.

### Why integrate into OpenPaw?

- OpenPaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openpaw memory recall "…" --k 25 --since 30d`
  - `openpaw memory reflect --since 7d`
...
Pragmatic take for `~/.openpaw/workspace`:
