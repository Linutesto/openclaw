---
summary: "Use MiniMax M2.1 in OpenPaw"
read_when:
  - You want MiniMax models in OpenPaw
...
```bash
openpaw plugins enable minimax-portal-auth  # skip if already loaded.
openpaw gateway restart  # restart if gateway is already running
openpaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/openpaw/openpaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openpaw.json`:
...
- Use `openpaw models list` and `openpaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `openpaw configure` and selecting **MiniMax M2.1**, or
