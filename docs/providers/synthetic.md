---
summary: "Use Synthetic's Anthropic-compatible API in OpenPaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenPaw registers it as the
...
```bash
openpaw onboard --auth-choice synthetic-api-key
```
...
Note: OpenPaw's Anthropic client appends `/v1` to the base URL, so use
