---
summary: "Use Venice AI privacy-focused models in OpenPaw"
read_when:
  - You want privacy-focused inference in OpenPaw
...
```bash
openpaw onboard --auth-choice venice-api-key
```
...
```bash
openpaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OpenPaw shows all available Venice models. Pick based on your needs:
...
```bash
openpaw models set venice/claude-opus-45
openpaw models set venice/llama-3.3-70b
```

List all available models:

```bash
openpaw models list | grep venice
```

## Configure via `openpaw configure`

1. Run `openpaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenPaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openpaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openpaw chat --model venice/claude-opus-45

# Use uncensored model
openpaw chat --model venice/venice-uncensored

# Use vision model with image
openpaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openpaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openpaw models list` to see currently available models. Some models may be temporarily offline.
