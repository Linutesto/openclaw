...
- You can run the CLI (`openpaw`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.openpaw/openpaw.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `openpaw.internal.`) on the gateway host and publish `_openclaw-gw._tcp` records.
...
1. Create `~/.openpaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
openpaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
