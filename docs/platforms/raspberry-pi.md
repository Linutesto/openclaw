---
summary: "OpenPaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenPaw on a Raspberry Pi
  - Running OpenPaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenPaw on Raspberry Pi

## Goal

Run a persistent, always-on OpenPaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenPaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openpaw

# Enable if not
sudo systemctl enable openpaw

# Start on boot
sudo systemctl start openpaw
```
...
```bash
# Check logs
journalctl -u openpaw --no-pager -n 100

# Common fix: rebuild
cd ~/openpaw  # if using hackable install
npm run build
sudo systemctl restart openpaw
```
