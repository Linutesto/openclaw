...
service via the external `openpaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPENPAW_PROFILE`; legacy `com.openpaw.*` is supported).
