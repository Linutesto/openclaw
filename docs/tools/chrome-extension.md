---
summary: "Chrome extension: let OpenPaw drive your existing Chrome tab"
read_when:
...
The OpenPaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openpaw-managed Chrome profile.
...
OpenPaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenPaw release (npm package) as static files. There is no separate “build” step.

After upgrading OpenPaw:

- Re-run `openpaw browser extension install` to refresh the installed files under your OpenPaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenPaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenPaw to control.
...
- `ON`: attached; OpenPaw can drive that tab.
...
Debugging: `openpaw sandbox explain`
...
`openpaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openpaw browser extension install` first to copy the extension to a stable location under your OpenPaw state directory.
...
- **This is not isolated** like the dedicated openpaw-managed profile.
