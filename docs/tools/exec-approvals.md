...
- **gateway host** → `openpaw` process on the gateway machine
...
`~/.openpaw/exec-approvals.json` directly.

CLI: `openpaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
