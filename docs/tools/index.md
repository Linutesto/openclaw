---
summary: "Agent tool surface for OpenPaw (browser, canvas, nodes, message, cron) replacing legacy `openpaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openpaw-*` skills
title: "Tools"
---

# Tools (OpenPaw)

OpenPaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openpaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openpaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenPaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`openpaw node run`).
...
- Requires a Brave API key (recommended: `openpaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenPaw-managed browser.
...
- Quick smoke: `openpaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openpaw gateway` restart in-place)
