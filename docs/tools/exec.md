...
- `gateway`/`node` approvals are controlled by `~/.openpaw/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  OpenPaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
