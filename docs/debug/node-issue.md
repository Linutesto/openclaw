...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenPaw
...
Running OpenPaw via Node with `tsx` fails at startup with:

```
[openpaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openpaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openpaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openpaw.mjs status` works on Node 25.
