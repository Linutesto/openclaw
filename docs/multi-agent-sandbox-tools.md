...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openpaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openpaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openpaw-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${OPENPAW_STATE_DIR:-$HOME/.openpaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
