...
```bash
openpaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openpaw tui --deliver`
...
- Check the Gateway logs: `openpaw logs --follow`.
- Confirm the agent can run: `openpaw status` and `openpaw models status`.
...
- No agents in picker: check `openpaw agents list` and your routing config.
