...
read_when:
  - "You installed OpenPaw but `openpaw` is “command not found”"
...
OpenPaw’s runtime baseline is **Node 22+**.

If you can run `npm install -g openpaw@latest` but later see `openpaw: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
