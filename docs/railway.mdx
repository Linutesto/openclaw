...
Deploy OpenPaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/openpaw` — Control UI
...
- `OPENPAW_STATE_DIR=/data/.openpaw` (recommended)
- `OPENPAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENPAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenPaw state + workspace so you can migrate to another host without losing config or memory.
