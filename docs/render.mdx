...
Deploy OpenPaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/openpaw/openpaw"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run OpenPaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENPAW_STATE_DIR
        value: /data/.openpaw
      - key: OPENPAW_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENPAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/openpaw`.
...
If you use the original OpenPaw repository, Render will not auto-deploy your OpenPaw. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenPaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenPaw host.
