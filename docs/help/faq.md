---
summary: "Frequently asked questions about OpenPaw setup, configuration, and usage"
title: "FAQ"
---
...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [What’s the recommended way to install and set up OpenPaw?](#whats-the-recommended-way-to-install-and-set-up-openpaw)
  ...
  - [I can't access docs.openpaw.ai (SSL error). What now?](#i-cant-access-docsopenclawai-ssl-error-what-now)
  - [What’s the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or openpaw not recognized](#windows-install-says-git-not-found-or-openpaw-not-recognized)
  - [The docs didn’t answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install OpenPaw on Linux?](#how-do-i-install-openpaw-on-linux)
  - [How do I install OpenPaw on a VPS?](#how-do-i-install-openpaw-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask OpenPaw to update itself?](#can-i-ask-openpaw-to-update-itself)
  ...
  - [If I buy a Mac mini to run OpenPaw, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-openpaw-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different OpenPaw instances?](#can-multiple-people-use-one-whatsapp-number-with-different-openpaw-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What’s the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run OpenPaw on a dedicated machine?](#how-important-is-it-to-run-openpaw-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run OpenPaw in a VM and what are the requirements](#can-i-run-openpaw-in-a-vm-and-what-are-the-requirements)
- [What is OpenPaw?](#what-is-openpaw)
  - [What is OpenPaw, in one paragraph?](#what-is-openpaw-in-one-paragraph)
  - [What’s the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for OpenPaw](#what-are-the-top-five-everyday-use-cases-for-openpaw)
  - [Can OpenPaw help with lead gen outreach ads and blogs for a SaaS](#can-openpaw-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can OpenPaw run tasks on a schedule or continuously in the background?](#can-openpaw-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with OpenPaw saved locally?](#is-all-data-used-with-openpaw-saved-locally)
  - [Where does OpenPaw store its data?](#where-does-openpaw-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What’s the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall OpenPaw?](#how-do-i-completely-uninstall-openpaw)
...
- [Config basics](#config-basics)
  ...
  - [Can the OpenPaw browser run headless?](#can-the-openpaw-browser-run-headless)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two OpenPaw instances talk to each other (local + VPS)?](#can-two-openpaw-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does OpenPaw load environment variables?](#how-does-openpaw-load-environment-variables)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of OpenPaw instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-openpaw-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset OpenPaw but keep it installed?](#how-do-i-completely-reset-openpaw-but-keep-it-installed)
  ...
  - [Why doesn’t OpenPaw reply in a group?](#why-doesnt-openpaw-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do OpenPaw, Flawd, and Krill use for models?](#what-do-openpaw-flawd-and-krill-use-for-models)
...
- [Gateway: ports, “already running”, and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `openpaw gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-openpaw-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `openpaw gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-openpaw-gateway-status-show-config-cli-and-config-service-different)
  - [What does “another gateway instance is already listening” mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run OpenPaw in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-openpaw-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart OpenPaw?](#i-closed-my-terminal-on-windows-how-do-i-restart-openpaw)
  ...
  - [ELI5: `openpaw gateway restart` vs `openpaw gateway`](#eli5-openpaw-gateway-restart-vs-openpaw-gateway)
  - [What’s the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media & attachments](#media-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose OpenPaw to inbound DMs?](#is-it-safe-to-expose-openpaw-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/openpaw/openpaw-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ```bash
   openpaw health --json
   openpaw health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://openpaw.ai/install.sh | bash -s -- --install-method git
```

This installs OpenPaw **from a git checkout**, so the agent can read the code + docs and
...
https://github.com/openpaw/openpaw/issues
https://github.com/openpaw/openpaw/pulls
...
- `openpaw status`: quick snapshot of gateway/agent health + basic config.
- `openpaw models status`: checks provider auth + model availability.
- `openpaw doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `openpaw status --all`, `openpaw logs --follow`,
`openpaw gateway status`, `openpaw health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up OpenPaw

The repo recommends running from source and using the onboarding wizard:

```bash
curl -fsSL https://openpaw.ai/install.sh | bash
openpaw onboard --install-daemon
```
...
If you don’t have a global install yet, run it via `pnpm openpaw onboard`.
...
- If it asks for auth, run `openpaw dashboard` and use the tokenized link (`?token=...`).
- The token is the same value as `gateway.auth.token` (or `OPENPAW_GATEWAY_TOKEN`) and is stored by the UI after first load.

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `openpaw gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `openpaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...` from `openpaw dashboard`.
...
1. Install OpenPaw on the new machine.
2. Copy `$OPENPAW_STATE_DIR` (default: `~/.openpaw`) from the old machine.
3. Copy your workspace (default: `~/.openpaw/workspace`).
4. Run `openpaw doctor` and restart the Gateway service.
...
under `~/.openpaw/` (for example `~/.openpaw/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-openpaw-store-its-data),
...
https://github.com/openpaw/openpaw/blob/main/CHANGELOG.md

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.openpaw.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.openpaw.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.openpaw.ai`, then retry. More
...
https://github.com/openpaw/openpaw/tree/main/docs
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://openpaw.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://openpaw.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
https://openpaw.ai/install.ps1
...
```bash
curl -fsSL https://openpaw.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://openpaw.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://openpaw.ai/install.sh | bash -s -- --install-method git --verbose
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or openpaw not recognized
...
**2) openpaw is not recognized after install**
...
### How do I install OpenPaw on Linux
...
### How do I install OpenPaw on a VPS
...
### Can I ask OpenPaw to update itself
...
```bash
openpaw update
openpaw update status
openpaw update --channel stable|beta|dev
openpaw update --tag <dist-tag|version>
openpaw update --no-restart
```

If you must automate from an agent:

```bash
openpaw update --yes --no-restart
openpaw gateway restart
```
...
`openpaw onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run OpenPaw with **API keys** (Anthropic/OpenAI/others) or with
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `openpaw models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `openpaw models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro/Max)

Yes — via **setup-token**. OpenPaw no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so OpenPaw can keep replying while a provider is rate‑limited.
...
OpenPaw supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. OpenPaw fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `openpaw.json`.

Steps:

1. Enable the plugin: `openpaw plugins enable google-gemini-cli-auth`
2. Login: `openpaw models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. OpenPaw needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. OpenPaw runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
any Mac works. OpenPaw’s iMessage integrations run on macOS (BlueBubbles or `imsg`), while
...
### If I buy a Mac mini to run OpenPaw can I connect it to my MacBook Pro
...
- Use `openpaw nodes status` / `openpaw nodes list` to see it.
...
- DM your bot, then run `openpaw logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different OpenPaw instances
...
If you run OpenPaw via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non‑login shells.
...
This **does not delete your data** - it only changes the OpenPaw code install. Your state
(`~/.openpaw`) and workspace (`~/.openpaw/workspace`) stay untouched.
...
**OpenPaw-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you’re actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run OpenPaw on a dedicated machine
...
OpenPaw is lightweight. For a basic Gateway + one chat channel:
...
### Can I run OpenPaw in a VM and what are the requirements
...
### What is OpenPaw in one paragraph

OpenPaw is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

OpenPaw is not “just a Claude wrapper.” It’s a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for OpenPaw
...
### Can OpenPaw help with lead gen outreach ads and blogs for a SaaS
...
OpenPaw draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

OpenPaw is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use OpenPaw when you
...
Showcase: https://openpaw.ai/showcase

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.openpaw/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.openpaw/openpaw.json`). Precedence is `<workspace>/skills` > `~/.openpaw/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.openpaw/openpaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openpaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which OpenPaw treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `OPENPAW_SKIP_CRON` is not set.
...
```bash
openpaw cron run <jobId> --force
openpaw cron runs --id <jobId> --limit 50
```
...
### Can OpenPaw run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.openpaw.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `imsg`, `apple-notes`, `apple-reminders`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**  
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-openpaw-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**  
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. OpenPaw can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.openpaw/skills`) to allow Linux:
   ```markdown
   ---
   name: imsg
   description: iMessage/SMS CLI for listing chats, history, watch, and sending.
   metadata: { "openpaw": { "os": ["darwin", "linux"], "requires": { "bins": ["imsg"] } } }
   ---
   ```
4. Start a new session so the skills snapshot refreshes.

For iMessage specifically, you can also point `channels.imessage.cliPath` at an SSH wrapper (OpenPaw only needs stdio). See [iMessage](/channels/imessage).
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured OpenPaw workspace); OpenPaw treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.openpaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
```bash
openpaw browser extension install
openpaw browser extension path
```
...
OpenPaw memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

OpenPaw also runs a **silent pre-compaction memory flush** to remind the model
...
If you don’t set a provider explicitly, OpenPaw auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, OpenPaw
prefers `local`.
...
### Is all data used with OpenPaw saved locally

No - **OpenPaw’s state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.openpaw` + your workspace directory).
...
### Where does OpenPaw store its data

Everything lives under `$OPENPAW_STATE_DIR` (default: `~/.openpaw`):

| Path                                                            | Purpose                                                      |
| --------------------------------------------------------------- | ------------------------------------------------------------ |
| `$OPENPAW_STATE_DIR/openpaw.json`                             | Main config (JSON5)                                          |
| `$OPENPAW_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use) |
| `$OPENPAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$OPENPAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
| `$OPENPAW_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$OPENPAW_STATE_DIR/agents/`                                   | Per‑agent state (agentDir + sessions)                        |
| `$OPENPAW_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                     |
| `$OPENPAW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single‑agent path: `~/.openpaw/agent/*` (migrated by `openpaw doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.openpaw/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.openpaw`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.openpaw`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.openpaw/skills`).

Default workspace is `~/.openpaw/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.openpaw/workspace" } },
}
```
...
Do **not** commit anything under `~/.openpaw` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall OpenPaw
...
`workspace` to the repo root. The OpenPaw repo is just source code; keep the
...
OpenPaw reads an optional **JSON5** config from `$OPENPAW_CONFIG_PATH` (default: `~/.openpaw/openpaw.json`):

```
$OPENPAW_CONFIG_PATH
```

If the file is missing, it uses safe‑ish defaults (including a default workspace of `~/.openpaw/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `OPENPAW_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `openpaw doctor --generate-gateway-token`.
...
key. **Recommended:** run `openpaw configure --section web` to store it in
...
- Daemons read env vars from `~/.openpaw/.env` (or the service environment).
...
### Can the OpenPaw browser run headless
...
5. Approve the node on the Gateway:
   ```bash
   openpaw nodes pending
   openpaw nodes approve <requestId>
   ```
...
- Gateway is running: `openpaw gateway status`
- Gateway health: `openpaw status`
- Channel health: `openpaw channels status`
...
### Can two OpenPaw instances talk to each other local VPS
...
`openpaw agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
openpaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.openpaw/openpaw.json`).
- If you have no backup, re-run `openpaw doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `openpaw config set` for small changes.
- Use `openpaw configure` for interactive edits.
...
```bash
openpaw gateway --tailscale serve
```
...
### How does OpenPaw load environment variables

OpenPaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.openpaw/.env` (aka `$OPENPAW_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.openpaw/.env` so they’re picked up even when the service doesn’t inherit your shell env.
...
`OPENPAW_LOAD_SHELL_ENV=1`, `OPENPAW_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`openpaw models status` reports whether **shell env import** is enabled. “Shell env: off”
does **not** mean your env vars are missing - it just means OpenPaw won’t load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won’t inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.openpaw/.env`:
   ```
...
### Is there a way to make a team of OpenPaw instances one CEO and many agents
...
### How do I completely reset OpenPaw but keep it installed
...
```bash
openpaw reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `OPENPAW_PROFILE`), reset each state dir (defaults are `~/.openpaw-<profile>`).
- Dev reset: `openpaw gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), OpenPaw skips the heartbeat run to save API calls.
...
No. OpenPaw runs on **your own account**, so if you’re in the group, OpenPaw can see it.
...
```bash
openpaw directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt OpenPaw reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.openpaw/agents/<agentId>/sessions/`.
...
- Use `openpaw doctor` to spot stray workspaces and profile mismatches.
...
OpenPaw’s default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider, OpenPaw currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `openpaw models set ...` (updates just model config)
- `openpaw configure --section models` (interactive)
- edit `agents.defaults.model` in `~/.openpaw/openpaw.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `openpaw doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do OpenPaw, Flawd, and Krill use for models

- **OpenPaw + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-5`) - see [Anthropic](/providers/anthropic).
...
Yes. OpenPaw ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.openpaw/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `openpaw agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so OpenPaw can keep responding even when a provider is rate‑limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.openpaw/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.openpaw/agent/*` (migrated by `openpaw doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.openpaw/.env` or enable `env.shellEnv`.
- **Make sure you’re editing the correct agent**
  - Multi‑agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity‑check model/auth status**
  - Use `openpaw models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `openpaw models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `openpaw models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.openpaw/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:
    ```bash
    openpaw models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), OpenPaw will try it during model fallback. If you haven’t configured Google credentials, you’ll see `No API key found for provider "google"`.
...
Fix: OpenPaw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
OpenPaw uses provider‑prefixed IDs like:
...
OpenPaw may temporarily skip a profile if it’s in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `openpaw models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent’s `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
openpaw models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
openpaw models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
openpaw models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
openpaw models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key whats the difference

OpenPaw supports both:
...
```
--port > OPENPAW_GATEWAY_PORT > gateway.port > default 18789
```

### Why does openpaw gateway status say Runtime running but RPC probe failed

Because “running” is the **supervisor’s** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `openpaw gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what’s actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn’t listening)

### Why does openpaw gateway status show Config cli and Config service different

You’re editing one config file while the service is running another (often a `--profile` / `OPENPAW_STATE_DIR` mismatch).

Fix:

```bash
openpaw gateway install --force
```
...
OpenPaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `openpaw gateway --port <port>`.

### How do I run OpenPaw in remote mode client connects to a Gateway elsewhere
...
- `openpaw gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `openpaw.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:

- Fastest: `openpaw dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don’t have a token yet: `openpaw doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...`.
- Set `gateway.auth.token` (or `OPENPAW_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token (or refresh with a one-time `?token=...` link).
- Still stuck? Run `openpaw status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `OPENPAW_CONFIG_PATH` (per‑instance config)
- `OPENPAW_STATE_DIR` (per‑instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `openpaw --profile <name> …` per instance (auto-creates `~/.openpaw-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `openpaw --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.openpaw.*`, `openpaw-gateway-<profile>.service`, `OpenPaw Gateway (<profile>)`).
...
```
openpaw tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/openpaw/openpaw-YYYY-MM-DD.log
```
...
- macOS: `$OPENPAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.openpaw/logs/...`; profiles use `~/.openpaw-<profile>/logs/...`)
- Linux: `journalctl --user -u openpaw-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "OpenPaw Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `openpaw gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart OpenPaw
...
1. Is the Gateway running? `openpaw gateway status`
2. Is the Gateway healthy? `openpaw status`
3. Does the UI have the right token? `openpaw dashboard`
...
```bash
openpaw channels status
openpaw channels logs --channel telegram
```
...
### ELI5 openpaw gateway restart vs openpaw gateway

- `openpaw gateway restart`: restarts the **background service** (launchd/systemd).
- `openpaw gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `openpaw gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [OpenPaw assistant setup](/start/openpaw) and [Agent send](/tools/agent-send).

CLI sending:

```bash
openpaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose OpenPaw to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM‑capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `openpaw pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `openpaw pairing list <channel>` if a code didn’t arrive.
- Opening DMs publicly requires explicit opt‑in (`dmPolicy: "open"` and allowlist `"*"`).

Run `openpaw doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. OpenPaw only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
openpaw pairing approve whatsapp <code>
```

List pending requests:

```bash
openpaw pairing list whatsapp
```
...
OpenPaw blocks **cross‑provider** messaging by default. If a tool call is bound
...
**A:** In OpenPaw, credentials and model selection are separate. Setting `ANTHROPIC_API_KEY` (or storing an Anthropic API key in auth profiles) enables authentication, but the actual default model is whatever you configure in `agents.defaults.model.primary` (for example, `anthropic/claude-sonnet-4-5` or `anthropic/claude-opus-4-5`). If you see `No credentials found for profile "anthropic:default"`, it means the Gateway couldn’t find Anthropic credentials in the expected `auth-profiles.json` for the agent that’s running.

---

Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/openpaw/openpaw/discussions).
