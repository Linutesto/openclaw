...
You can also set `OPENPAW_VERBOSE=1` instead of the flag.
...
### `docs.openpaw.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.openpaw.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.openpaw.ai` to the allowlist, then retry.
...
- Check the allowlist: `openpaw config get agents.defaults.models`
...
If you can, include the relevant log tail from `openpaw logs --follow`.
