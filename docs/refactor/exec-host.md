...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.openpaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.openpaw/node.json` (node id + pairing token).
...
Path: `~/.openpaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openpaw/exec-approvals.sock",
...
- Unix socket at `~/.openpaw/exec-approvals.sock` (0600).
