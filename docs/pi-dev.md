...
This guide summarizes a sane workflow for working on the pi integration in OpenPaw.
...
- Trigger the agent directly:
  - `pnpm openpaw agent --message "Hello" --thinking low`
...
State lives under the OpenPaw state directory. Default is `~/.openpaw`. If `OPENPAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `openpaw.json` for config
...
- https://docs.openpaw.ai/testing
- https://docs.openpaw.ai/start/getting-started
