...
read_when:
  - You want to understand what “context” means in OpenPaw
...
“Context” is **everything OpenPaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OpenPaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OpenPaw builds the system prompt

The system prompt is **OpenPaw-owned** and rebuilt each run. It includes:
...
By default, OpenPaw injects a fixed set of workspace files (if present):
