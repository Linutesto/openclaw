...
- `/usage cost` in chats: local cost summary aggregated from OpenPaw session logs.
- CLI: `openpaw status --usage` prints a full per-provider breakdown.
- CLI: `openpaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
