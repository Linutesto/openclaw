---
summary: "Quick troubleshooting guide for common OpenPaw failures"
read_when:
...
When OpenPaw misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `openpaw status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `openpaw status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `openpaw status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `openpaw gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `openpaw channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `openpaw gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `openpaw logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `openpaw status --all` (it redacts tokens). If you paste `openpaw status`, consider setting `OPENPAW_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  openpaw models auth setup-token --provider anthropic
  ```
...
```bash
openpaw models auth paste-token --provider anthropic
openpaw models status
```
...
- Preferred: `openpaw logs --follow`
- File logs (always): `/tmp/openpaw/openpaw-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$OPENPAW_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u openpaw-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  openpaw config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  openpaw config set gateway.mode remote
  openpaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `openpaw setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.openpaw/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `openpaw doctor`
...
- then run `openpaw sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `openpaw gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `openpaw gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `openpaw gateway status` and `openpaw doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `OPENPAW_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `openpaw gateway status` shows a config mismatch**
...
- Fix: rerun `openpaw gateway install --force` from the same `--profile` / `OPENPAW_STATE_DIR` you want the service to use.

**If `openpaw gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `openpaw doctor` to update it (or `openpaw gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `OPENPAW_GATEWAY_TOKEN`) and restart the service.

**If `openpaw gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/openpaw` on disk.
...
Symptoms: `pwd` or file tools show `~/.openpaw/sandboxes/...` even though you
...
OpenPaw intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `openpaw models list` or
  `openpaw models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENPAW_CONFIG_PATH:-$HOME/.openpaw/openpaw.json}"
```
...
tail -f "$(ls -t /tmp/openpaw/openpaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
openpaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.openpaw/agents/<agentId>/sessions/
```
...
openpaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
trash "${OPENPAW_STATE_DIR:-$HOME/.openpaw}/credentials" # if logout can't cleanly remove everything
openpaw channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/openpaw/openpaw-*.log | head -1)" | tail -20
```

### High Memory Usage

OpenPaw keeps conversation history in memory.
...
OpenPaw now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `openpaw doctor` reports every invalid entry.
- `openpaw doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `openpaw logs`, `openpaw health`, `openpaw status`, `openpaw gateway status`, and `openpaw gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/openpaw/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `openpaw models status` (CLI).
...
```bash
curl -fsSL https://openpaw.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `openpaw channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. OpenPaw scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/openpaw/openpaw/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.openpaw.* still works)
```
...
Ensure the global `openpaw` CLI is installed and matches the app version:

```bash
openpaw --version
npm install -g openpaw@<version>
```
...
#   ${OPENPAW_CONFIG_PATH:-$HOME/.openpaw/openpaw.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
openpaw gateway --verbose
openpaw channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/openpaw/openpaw-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$OPENPAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.openpaw/logs/...`; profiles use `~/.openpaw-<profile>/logs/...`)<br />Linux: `journalctl --user -u openpaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "OpenPaw Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$OPENPAW_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$OPENPAW_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$OPENPAW_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
tail -20 /tmp/openpaw/openpaw-*.log
```
...
trash "${OPENPAW_STATE_DIR:-$HOME/.openpaw}"
openpaw channels login         # re-pair WhatsApp
openpaw gateway restart           # or: openpaw gateway
```
...
1. Check logs first: `/tmp/openpaw/` (default: `openpaw-YYYY-MM-DD.log`, or your configured `logging.file`)
