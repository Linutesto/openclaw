---
summary: "All configuration options for ~/.openpaw/openpaw.json with examples"
read_when:
...
OpenPaw reads an optional **JSON5** config from `~/.openpaw/openpaw.json` (comments + trailing commas allowed).

If the file is missing, OpenPaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.openpaw/workspace`). You usually only need a config to:
...
OpenPaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `openpaw doctor`, `openpaw logs`, `openpaw health`, `openpaw status`, `openpaw service`, `openpaw help`).
- Run `openpaw doctor` to see the exact issues.
- Run `openpaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `openpaw config set`. Keep a backup of `~/.openpaw/openpaw.json`.
...
```bash
openpaw gateway call config.get --params '{}' # capture payload.hash
openpaw gateway call config.apply --params '{
...
```bash
openpaw gateway call config.get --params '{}' # capture payload.hash
openpaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.openpaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@openpaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.openpaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.openpaw/workspace" }],
}
```
...
{ "$include": "/etc/openpaw/base.json5" } // absolute
...
```json5
// ~/.openpaw/clients/mueller/agents.json5
[
...
```json5
// ~/.openpaw/clients/mueller/broadcast.json5
{
...
OpenPaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, OpenPaw runs your login shell and imports only the missing expected keys (never overrides).
...
OpenPaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.openpaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.openpaw/credentials/oauth.json` (or `$OPENPAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.openpaw/agent/*` (migrated by `openpaw doctor` into `~/.openpaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `OPENPAW_OAUTH_DIR`
- Agent dir (default agent root override): `OPENPAW_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, OpenPaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, OpenPaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/openpaw/openpaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/openpaw/openpaw.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.openpaw/credentials/whatsapp/biz
          // authDir: "~/.openpaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `openpaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openpaw", "openpaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@openpaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.openpaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.openpaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.openpaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.openpaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.openpaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.openpaw/workspace-home" },
      { id: "work", workspace: "~/.openpaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `openpaw.json`).
...
OpenPaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["openpaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-openpaw",
...
OpenPaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `openpaw plugins install @openpaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
OpenPaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
OpenPaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, OpenPaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/openpaw" } },
}
```
...
When a file exceeds this limit, OpenPaw logs a warning and injects a truncated
...
message envelopes). If unset, OpenPaw uses the host timezone at runtime.
...
`[openpaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[openpaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[openpaw]"`, OpenPaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, OpenPaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.openpaw/settings/tts.json",
      elevenlabs: {
...
OpenPaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, OpenPaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `openpaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:openpaw`: all built-in OpenPaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.openpaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.openpaw/sandboxes",
        docker: {
          image: "openpaw-sandbox:bookworm-slim",
          containerPrefix: "openpaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "openpaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "openpaw-sandbox-browser:bookworm-slim",
          containerPrefix: "openpaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenPaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.openpaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
OpenPaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, OpenPaw writes/merges a `models.json` into
`~/.openpaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. OpenPaw uses
...
- Shortcut: `openpaw onboard --auth-choice opencode-zen`.
...
Shortcut: `openpaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `openpaw onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `openpaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `openpaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `openpaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENPAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.openpaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.openpaw/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.openpaw/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, OpenPaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.openpaw/skills` (workspace skills
...
from `~/.openpaw/extensions`, `<workspace>/.openpaw/extensions`, plus any
...
### `browser` (openpaw-managed browser)

OpenPaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for openpaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (OpenPaw.app menubar, or `openpaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      openpaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/openpaw" }
...
- Examples: `"/ui"`, `"/openpaw"`, `"/apps/openpaw"`.
...
- When a connection comes from one of these IPs, OpenPaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `openpaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OPENPAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OPENPAW_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. OpenPaw
...
- OpenPaw.app watches `~/.openpaw/openpaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.openpaw/openpaw.json` (or `OPENPAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.openpaw/openpaw.json` (or `OPENPAW_CONFIG_PATH`)
...
- `OPENPAW_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `openpaw --dev …` → uses `~/.openpaw-dev` + shifts ports from base `19001`
- `openpaw --profile <name> …` → uses `~/.openpaw-<name>` (port via config/env/flags)
...
```bash
OPENPAW_CONFIG_PATH=~/.openpaw/a.json \
OPENPAW_STATE_DIR=~/.openpaw-a \
openpaw gateway --port 19001
```
...
Gmail helper config (used by `openpaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "openpaw@gmail.com",
...
- Set `OPENPAW_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, OpenPaw defaults `serve.path` to `/` so
...
Default root: `~/.openpaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the openpaw browser CDP port `18792`)  
...
- env: `OPENPAW_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.openpaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.openpaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.openpaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
- Hostname: defaults to `openpaw` (advertises `openpaw.local`). Override with `OPENPAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-gw._tcp` under `~/.openpaw/dns/` using the configured discovery domain (example: `openpaw.internal.`).
