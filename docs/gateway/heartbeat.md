...
- During heartbeat runs, OpenPaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openpaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenPaw skips the heartbeat run entirely (no model call).
...
```bash
openpaw system event --text "Check for urgent follow-ups" --mode now
```
