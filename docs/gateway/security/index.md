...
## Quick check: `openpaw security audit`
...
openpaw security audit --deep
openpaw security audit --fix
```
...
- Tighten local perms (`~/.openpaw` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

OpenPaw is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, OpenPaw also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.openpaw/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.openpaw/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.openpaw/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.openpaw/credentials/oauth.json`
...
`openpaw security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENPAW_GATEWAY_PASSWORD}
```
...
OpenPaw stores session transcripts on disk under `~/.openpaw/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.openpaw` (see the audit section below). If you need
...
OpenPaw can refresh the skills list mid-session:
...
- If you install plugins from npm (`openpaw plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openpaw/extensions/<pluginId>/` (or `$OPENPAW_STATE_DIR/extensions/<pluginId>/`).
  - OpenPaw uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
openpaw pairing approve <channel> <code>
```
...
By default, OpenPaw routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenPaw has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.openpaw/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.openpaw or your logs.”
...
4. **Re-run audit**
   - `openpaw security audit --deep` and confirm the report is clean.
...
- `~/.openpaw/openpaw.json`: `600` (user read/write only)
- `~/.openpaw`: `700` (user only)

`openpaw doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENPAW_GATEWAY_PORT`
...
4. **Environment variable** (alternative): set `OPENPAW_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openpaw doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENPAW_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `OPENPAW_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenPaw
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. OpenPaw verifies the identity by resolving the
...
- OpenPaw will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openpaw/` (or `$OPENPAW_STATE_DIR/`) may contain secrets or private data:

- `openpaw.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openpaw status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openpaw", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openpaw/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openpaw` profile).
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openpaw gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENPAW_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/openpaw/openpaw-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openpaw/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + OpenPaw version
...
Found a vulnerability in OpenPaw? Please report responsibly:

1. Email: security@openpaw.ai
