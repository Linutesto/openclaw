...
`openpaw doctor` is the repair + migration tool for OpenPaw. It fixes stale
...
```bash
openpaw doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
openpaw doctor --repair --force
```
...
- Extra workspace dir detection (`~/openpaw`).
...
you to run `openpaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.openpaw/openpaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.openpaw/sessions/` to `~/.openpaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.openpaw/agent/` to `~/.openpaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.openpaw/credentials/*.json` (except `oauth.json`)
  - to `~/.openpaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `openpaw doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.openpaw` folders exist across
  home directories or when `OPENPAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.openpaw/openpaw.json` is
...
offers to remove them and install the OpenPaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named OpenPaw gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `openpaw doctor --generate-gateway-token` to force token
...
- `openpaw doctor` prompts before rewriting supervisor config.
- `openpaw doctor --yes` accepts the default repair prompts.
- `openpaw doctor --repair` applies recommended fixes without prompts.
- `openpaw doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `openpaw gateway install --force`.
