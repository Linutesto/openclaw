...
- Replaces the legacy `gateway` command. CLI entry point: `openpaw gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
openpaw gateway --port 18789
# for full debug/trace logs in stdio:
openpaw gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
openpaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.openpaw/openpaw.json` (or `OPENPAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openclaw__/canvas/` from `~/.openpaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENPAW_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OPENPAW_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OPENPAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.openpaw.*` may still exist)
- Linux: `openpaw-gateway-<profile>.service`
- Windows: `OpenPaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `OPENPAW_SERVICE_MARKER=openpaw`
- `OPENPAW_SERVICE_KIND=gateway`
- `OPENPAW_SERVICE_VERSION=<version>`
...
```bash
openpaw --dev setup
openpaw --dev gateway --allow-unconfigured
...
- `OPENPAW_STATE_DIR=~/.openpaw-dev`
- `OPENPAW_CONFIG_PATH=~/.openpaw-dev/openpaw.json`
- `OPENPAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.openpaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `OPENPAW_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OPENPAW_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `OPENPAW_CONFIG_PATH`
- unique `OPENPAW_STATE_DIR`
...
```bash
openpaw --profile main gateway install
openpaw --profile rescue gateway install
```

Example:

```bash
OPENPAW_CONFIG_PATH=~/.openpaw/a.json OPENPAW_STATE_DIR=~/.openpaw-a openpaw gateway --port 19001
OPENPAW_CONFIG_PATH=~/.openpaw/b.json OPENPAW_STATE_DIR=~/.openpaw-b openpaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `openpaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `openpaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `openpaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.openpaw.*` is cleaned up).
  - `openpaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are OpenPaw profile services.
  ...
  - Cleanup: `openpaw gateway uninstall` (current service) and `openpaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `openpaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- OpenPaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.openpaw.*` labels still unload cleanly).
- To stop it cleanly, use `openpaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `openpaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `openpaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

OpenPaw installs a **systemd user service** by default on Linux/WSL2. We
...
`openpaw gateway install` writes the user unit. `openpaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/openpaw-gateway[-<profile>].service`:

```
[Unit]
Description=OpenPaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/openpaw gateway --port 18789
Restart=always
RestartSec=5
Environment=OPENPAW_GATEWAY_TOKEN=
...
```
systemctl --user enable --now openpaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/openpaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now openpaw-gateway[-<profile>].service
```
...
- `openpaw gateway health|status` — request health/status over the Gateway WS.
- `openpaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `openpaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `openpaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `openpaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `openpaw gateway` and the legacy TCP control port.
