...
function trySetupCompletion(repoRoot) {
  ...
  if (process.env.CI || process.env.OPENPAW_SKIP_COMPLETION_SETUP) {
    return;
  }

  const binPath = path.join(repoRoot, "openpaw.mjs");
  ...
  try {
    // Run with OPENPAW_SKIP_POSTINSTALL to avoid any weird recursion,
    // though distinct from this script.
    spawnSync(process.execPath, [binPath, "completion", "--install", "--yes"], {
      cwd: repoRoot,
      stdio: "inherit",
      env: { ...process.env, OPENPAW_SKIP_POSTINSTALL: "1" },
    });
  } catch {
...
try {
  const skip =
    process.env.OPENPAW_SKIP_POSTINSTALL === "1" ||
