...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openpaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenPaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/openpaw/scripts/mobile-reauth.sh'" --id openpaw-auth
        ;;
    *)
