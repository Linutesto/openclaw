...
ZIP=${1:?"Usage: $0 OpenPaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/openpaw/openpaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpenPaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/openpaw/openpaw/releases/download/v${VERSION}/"}
