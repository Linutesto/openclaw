...
BASE_IMAGE="${BASE_IMAGE:-openpaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openpaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openpaw.sandbox=1)
NOTE
