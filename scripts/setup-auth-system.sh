#!/bin/bash
# Setup OpenPaw Auth Management System
...
echo "=== OpenPaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openpaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenPaw message: Send warning via OpenPaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openpaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openpaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openpaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openpaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openpaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
