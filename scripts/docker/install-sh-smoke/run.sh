...
INSTALL_URL="${OPENPAW_INSTALL_URL:-https://openpaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENPAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENPAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openpaw"
PACKAGE_NAME="${OPENPAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENPAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENPAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENPAW_INSTALL_LATEST_OUT:-}"
fi
