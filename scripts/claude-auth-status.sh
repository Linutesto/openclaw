...
# Checks both Claude Code and OpenPaw auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
OPENPAW_AUTH="$HOME/.openpaw/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    openpaw models status --json 2>/dev/null || true
}
...
check_openclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$OPENPAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        openclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$OPENPAW_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            openpaw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
echo ""
echo "OpenPaw Auth (~/.openpaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$OPENPAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$OPENPAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'openpaw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active openpaw >/dev/null 2>&1; then
    echo -e "OpenPaw service: ${GREEN}running${NC}"
else
    echo -e "OpenPaw service: ${RED}NOT running${NC}"
fi
