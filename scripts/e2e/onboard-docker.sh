...
IMAGE_NAME="openpaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/openpaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/openpaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/openpaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/openpaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/openpaw"
    config_path="$HOME/.openpaw/openpaw.json"
    sessions_dir="$HOME/.openpaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.openpaw/openpaw.json" <<'"'"'JSON'"'"'
{
