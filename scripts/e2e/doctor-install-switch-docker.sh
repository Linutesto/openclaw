...
IMAGE_NAME="openpaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/openpaw-bin:$PATH"
  mkdir -p /tmp/openpaw-bin

  cat > /tmp/openpaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/openpaw-bin/systemctl

  cat > /tmp/openpaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/openpaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/openpaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/openpaw/openpaw.mjs"
  git_entry="/app/openpaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/openpaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/openpaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
