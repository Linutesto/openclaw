...
IMAGE_NAME="openpaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openpaw-net-e2e-$$"
GW_NAME="openpaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENPAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENPAW_SKIP_CHANNELS=1" \
	  -e "OPENPAW_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENPAW_SKIP_CRON=1" \
	  -e "OPENPAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
