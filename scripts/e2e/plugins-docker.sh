...
IMAGE_NAME="openpaw-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/openpaw-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.openpaw/extensions"

  cat > "$HOME/.openpaw/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/openpaw-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@openpaw/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/openpaw-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/openpaw-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@openpaw/demo-plugin-file",
  "version": "0.0.1",
  "openpaw": { "extensions": ["./index.js"] }
}
JSON
