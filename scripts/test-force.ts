...
function runTests() {
  const isolatedLock =
    process.env.OPENPAW_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `openpaw-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      OPENPAW_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.OPENPAW_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
