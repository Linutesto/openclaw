...
show_usage() {
    cat << EOF
clawlog - OpenPaw Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View OpenPaw logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    OpenPaw logs flow through the macOS unified log (subsystem: ai.openpaw).
