...
#
# Suggested cron: */30 * * * * /home/admin/openpaw/scripts/auth-monitor.sh
#
...
STATE_FILE="$HOME/.openpaw/auth-monitor-state"
...
send_notification() {
    ...
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openpaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenPaw to $NOTIFY_PHONE..."
            openpaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenPaw is down. Run: ssh l36 '~/openpaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
