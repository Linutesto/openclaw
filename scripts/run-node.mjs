...
const compilerOverride = env.OPENPAW_TS_COMPILER ?? env.CLAWDBOT_TS_COMPILER;
...
const logRunner = (message) => {
  if (env.OPENPAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[openpaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["openpaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
