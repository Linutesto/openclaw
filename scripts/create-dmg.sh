...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "OpenPaw")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "OpenPaw")
...
DMG_TEMP="$(mktemp -d /tmp/openpaw-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/OpenPaw/Resources/OpenPaw.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/openpaw-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/openpaw-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/openpaw-dmg-limits.txt
