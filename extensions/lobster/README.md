...
- This plugin integrates Lobster with OpenPaw _without core changes_.
...
## Using `openpaw.invoke` (Lobster → OpenPaw tools)

Some Lobster pipelines may include a `openpaw.invoke` step to call back into OpenPaw tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).
