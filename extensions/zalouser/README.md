...
OpenPaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
openpaw channels login --channel zalouser
...
```bash
openpaw message send --channel zalouser --target <threadId> --message "Hello from OpenPaw!"
```
...
```bash
openpaw channels login --channel zalouser              # Login via QR
openpaw channels login --channel zalouser --account work
openpaw channels status --probe
openpaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
openpaw directory self --channel zalouser
openpaw directory peers list --channel zalouser --query "name"
openpaw directory groups list --channel zalouser --query "work"
openpaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
openpaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
openpaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `openpaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
openpaw channels login --channel zalouser --account work
openpaw message send --channel zalouser --account work --target <id> --message "Hello"
