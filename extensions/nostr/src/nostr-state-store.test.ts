...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENPAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-nostr-"));
  process.env.OPENPAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".openpaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENPAW_STATE_DIR = previous;
    }
