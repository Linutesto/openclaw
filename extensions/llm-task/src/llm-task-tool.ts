...
// NOTE: This extension is intended to be bundled with OpenPaw.
// When running from source (tests/dev), OpenPaw internals live under src/.
...
export function createLlmTaskTool(api: OpenClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via openpaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-llm-task-"));
