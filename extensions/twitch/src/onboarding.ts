...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: OPENPAW_TWITCH_ACCESS_TOKEN",
...
async function configureWithEnvToken(
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var OPENPAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.OPENPAW_TWITCH_ACCESS_TOKEN?.trim();
