...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openpaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openpaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENPAW_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OPENPAW_GATEWAY_TOKEN: inst.gatewayToken,
        OPENPAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENPAW_GATEWAY_PORT: String(gwA.port),
          OPENPAW_GATEWAY_TOKEN: gwA.gatewayToken,
          OPENPAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENPAW_GATEWAY_PORT: String(gwB.port),
          OPENPAW_GATEWAY_TOKEN: gwB.gatewayToken,
          OPENPAW_GATEWAY_PASSWORD: "",
        }),
