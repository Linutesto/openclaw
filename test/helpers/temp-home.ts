...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENPAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OPENPAW_STATE_DIR = path.join(base, ".openpaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openpaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openpaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
