...
```bash
cat ~/.openpaw/openpaw.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.openpaw/openpaw.json | jq '.gateway.bind'`
...
Node is offline. Use `openpaw nodes list` to find online nodes.
