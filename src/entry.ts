...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OPENPAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OPENPAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OPENPAW_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[openpaw] ${parsed.error}`);
    process.exit(2);
  }
