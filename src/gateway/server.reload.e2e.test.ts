...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENPAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENPAW_SKIP_GMAIL_WATCHER;
    process.env.OPENPAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENPAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENPAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENPAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
