...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENPAW_STATE_DIR;
  previousConfigPath = process.env.OPENPAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENPAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENPAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENPAW_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENPAW_STATE_DIR = path.join(tempHome, ".openpaw");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "openpaw-test-"))
    : path.join(tempHome, ".openpaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPENPAW_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENPAW_CONFIG_PATH;
    } else {
      process.env.OPENPAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENPAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENPAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENPAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENPAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENPAW_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENPAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.OPENPAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENPAW_GATEWAY_TOKEN = fallbackToken;
  }
