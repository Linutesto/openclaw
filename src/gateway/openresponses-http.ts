 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for OpenPaw Gateway.
 *
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from OpenPaw.";
