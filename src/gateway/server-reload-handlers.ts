  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.OPENPAW_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (OPENPAW_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (OPENPAW_SKIP_CHANNELS=1 or OPENPAW_SKIP_PROVIDERS=1)",
        );
      } else {
