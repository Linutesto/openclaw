...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.OPENPAW_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-gw-mock-home-"));
      ...
      process.env.OPENPAW_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.OPENPAW_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "openpaw");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.openpaw-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".openpaw");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "openpaw.json");
      ...
      process.env.OPENPAW_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.OPENPAW_CONFIG_PATH = prev.configPath;
        process.env.OPENPAW_GATEWAY_TOKEN = prev.token;
        process.env.OPENPAW_SKIP_CHANNELS = prev.skipChannels;
        process.env.OPENPAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.OPENPAW_SKIP_CRON = prev.skipCron;
        process.env.OPENPAW_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.OPENPAW_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.OPENPAW_STATE_DIR = prev.stateDir;
      process.env.OPENPAW_CONFIG_PATH = prev.configPath;
      process.env.OPENPAW_GATEWAY_TOKEN = prev.token;
      process.env.OPENPAW_SKIP_CHANNELS = prev.skipChannels;
      process.env.OPENPAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
