...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `openpaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENPAW_CONFIG_PATH = path.join(root, "openpaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "openpaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "openpaw-gateway-test"),
        ...fileDefaults,
