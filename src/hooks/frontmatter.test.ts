...
`;
    ...
    expect(parsed.openpaw.emoji).toBe("💾");
    expect(parsed.openpaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openpaw.emoji).toBe("📝");
    expect(parsed.openpaw.events).toEqual(["command"]);
    expect(parsed.openpaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openpaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openpaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openpaw": {"events": ["test"]}}');
  });
...
describe("resolveOpenPawMetadata", () => {
  it("extracts openpaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenPawMetadata(frontmatter);
  ...
  it("returns undefined when openpaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openpaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenPaw" },
            { id: "npm", kind: "npm", package: "@openpaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openpaw/hook");
  });
    ...
    "openpaw":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OpenPaw" }],
      },
  }
---
...
`;
    ...
    const openpaw = resolveOpenPawMetadata(frontmatter);
    expect(openpaw).toBeDefined();
    expect(openpaw?.emoji).toBe("💾");
    expect(openpaw?.events).toEqual(["command:new"]);
    expect(openpaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(openpaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openpaw = resolveOpenPawMetadata(frontmatter);
    expect(openpaw?.emoji).toBe("disk");
    expect(openpaw?.events).toEqual(["command:new"]);
  });
});
