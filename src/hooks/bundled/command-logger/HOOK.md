...
homepage: https://docs.openpaw.ai/hooks#command-logger
metadata:
  {
    "openpaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openpaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openpaw/logs/commands.log ~/.openpaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openpaw`:
   ```
   /home/username/.openpaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
