...
This directory contains hooks that ship with OpenPaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.openpaw.ai/hooks/soul-evil
...
Hooks can be configured in `~/.openpaw/openpaw.json`:
...
2. Restart gateway: `pkill -9 -f 'openpaw.*gateway' && pnpm openpaw gateway`
3. Enable the hook: `openpaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openpaw.ai/hooks
