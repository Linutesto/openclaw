...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENPAW_GATEWAY_PORT).toBe("18789");
    expect(env.OPENPAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENPAW_SERVICE_MARKER).toBe("openpaw");
    expect(env.OPENPAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENPAW_SERVICE_VERSION).toBe("string");
    expect(env.OPENPAW_SYSTEMD_UNIT).toBe("openpaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENPAW_LAUNCHD_LABEL).toBe("ai.openpaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENPAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENPAW_SYSTEMD_UNIT).toBe("openpaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENPAW_LAUNCHD_LABEL).toBe("ai.openpaw.work");
    }
  });
});
