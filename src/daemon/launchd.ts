...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.OPENPAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(args?.env?.OPENPAW_PROFILE);
}
...
export function resolveGatewayLogPaths(env: Record<string, string | undefined>): {
  ...
  const prefix = env.OPENPAW_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.OPENPAW_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.OPENPAW_PROFILE)) {
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.OPENPAW_PROFILE,
      version: environment?.OPENPAW_SERVICE_VERSION ?? env.OPENPAW_SERVICE_VERSION,
    });
