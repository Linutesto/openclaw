...
export function triggerOpenPawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENPAW_SYSTEMD_UNIT,
        process.env.OPENPAW_PROFILE,
      );
  ...
  const label =
    process.env.OPENPAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENPAW_PROFILE);
