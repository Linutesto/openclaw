...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "openpaw doctor",
        managerScriptArgs(manager, "openpaw", ["doctor", "--non-interactive"]),
        gitRoot,
        { OPENPAW_UPDATE_IN_PROGRESS: "1" },
      ),
    );
