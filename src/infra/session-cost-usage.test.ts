...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-cost-"));
    ...
    const originalState = process.env.OPENPAW_STATE_DIR;
    process.env.OPENPAW_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.OPENPAW_STATE_DIR = originalState;
      }
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-cost-session-"));
