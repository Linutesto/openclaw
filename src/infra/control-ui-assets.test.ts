...
describe("control UI assets helpers", () => {
  it("resolves repo root from src argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-ui-"));
    try {
  ...
  it("resolves dist control-ui index path from package root argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-ui-"));
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "openpaw" }));
      await fs.writeFile(path.join(tmp, "openpaw.mjs"), "export {};\n");
      ...
      expect(await resolveControlUiDistIndexPath(path.join(tmp, "openpaw.mjs"))).toBe(
  ...
  it("resolves dist control-ui index path from .bin argv1", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(tmp, "node_modules", "openpaw");
      await fs.mkdir(binDir, { recursive: true });
      await fs.mkdir(path.join(pkgRoot, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(binDir, "openpaw"), "#!/usr/bin/env node\n");
      await fs.writeFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openpaw" }));
      await fs.writeFile(path.join(pkgRoot, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(binDir, "openpaw"))).toBe(
