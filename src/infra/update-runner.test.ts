...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-update-"));
  });
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "openpaw", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "openpaw", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm openpaw doctor --non-interactive": { stdout: "" },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "openpaw", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "openpaw");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g openpaw@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "openpaw", version: "2.0.0" }),
          "utf-8",
        );
    ...
    expect(calls.some((call) => call === "npm i -g openpaw@latest")).toBe(true);
  });
  ...
  it("updates global npm installs with tag override", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g openpaw@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "openpaw");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "openpaw", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g openpaw@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "openpaw", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g openpaw@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a openpaw checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-openpaw-root");
