...
function candidateBinDirs(opts: EnsureOpenPawPathOpts): string[] {
  ...
  // Bundled macOS app: `openpaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openpaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/openpaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openpaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openpaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenPawCliOnPath(opts: EnsureOpenPawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENPAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENPAW_PATH_BOOTSTRAPPED = "1";
