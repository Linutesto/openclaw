...
const SANDBOX_EXAMPLES = {
  main: [
    ["openpaw sandbox list", "List all sandbox containers."],
    ["openpaw sandbox list --browser", "List only browser containers."],
    ["openpaw sandbox recreate --all", "Recreate all containers."],
    ["openpaw sandbox recreate --session main", "Recreate a specific session."],
    ["openpaw sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openpaw sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openpaw sandbox list", "List all sandbox containers."],
    ["openpaw sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["openpaw sandbox recreate --all", "Recreate all containers."],
    ["openpaw sandbox recreate --session main", "Recreate a specific session."],
    ["openpaw sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openpaw sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openpaw sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openpaw sandbox explain", "Show effective sandbox config."],
    ["openpaw sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openpaw sandbox explain --agent work", "Explain an agent sandbox."],
    ["openpaw sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openpaw.ai/cli/sandbox")}\n`,
    )
    .action(() => {
