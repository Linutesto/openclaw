...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: OpenPawConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage OpenPaw plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.openpaw.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.openpaw.ai/plugin");
      lines.push("");
