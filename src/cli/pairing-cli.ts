  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!opts.channel && !code) {
        throw new Error(
          `Usage: ${formatCliCommand("openpaw pairing approve <channel> <code>")} (or: ${formatCliCommand("openpaw pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("openpaw pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
