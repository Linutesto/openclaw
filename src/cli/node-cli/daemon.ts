...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("openpaw node install"), formatCliCommand("openpaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("openpaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OPENPAW_LOG_PREFIX: baseEnv.OPENPAW_LOG_PREFIX ?? "node",
