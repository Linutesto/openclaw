...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openpaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENPAW_NO_RESPAWN: "1",
      OPENPAW_STATE_DIR: stateDir,
      OPENPAW_SKIP_CHANNELS: "1",
      OPENPAW_SKIP_GMAIL_WATCHER: "1",
      OPENPAW_SKIP_CRON: "1",
      OPENPAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENPAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openpaw-entry-bootstrap.mjs");
