...
export function registerAgentCommands(program: Command, args: { agentChannelOptions: string }) {
  program
    ...
    .option(
      "--auto-tools",
      "Execute tool calls automatically and loop until no more tool calls",
      false,
    )
    .option(
      "--max-tool-iterations <n>",
      "Maximum tool iterations when --auto-tools is enabled (default: 10)",
    )
...
${formatHelpExamples([
  ['openpaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openpaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openpaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openpaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openpaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openpaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openpaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openpaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openpaw agents set-identity --agent main --name "OpenPaw" --emoji "🦞"', "Set name + emoji."],
  ["openpaw agents set-identity --agent main --avatar avatars/openpaw.png", "Set avatar path."],
  [
    "openpaw agents set-identity --workspace ~/.openpaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openpaw agents set-identity --identity-file ~/.openpaw/workspace/IDENTITY.md --agent main",
