...
export async function installCompletion(shell: string, yes: boolean, binName = "openpaw") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c openpaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'openpaw gateway' and not 'openpaw other gateway', we need to check the sequence.
