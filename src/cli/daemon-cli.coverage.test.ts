...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OPENPAW_STATE_DIR: process.env.OPENPAW_STATE_DIR,
    OPENPAW_CONFIG_PATH: process.env.OPENPAW_CONFIG_PATH,
    OPENPAW_GATEWAY_PORT: process.env.OPENPAW_GATEWAY_PORT,
    OPENPAW_PROFILE: process.env.OPENPAW_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENPAW_STATE_DIR = "/tmp/openpaw-cli-state";
    process.env.OPENPAW_CONFIG_PATH = "/tmp/openpaw-cli-state/openpaw.json";
    delete process.env.OPENPAW_GATEWAY_PORT;
    delete process.env.OPENPAW_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENPAW_STATE_DIR !== undefined) {
      process.env.OPENPAW_STATE_DIR = originalEnv.OPENPAW_STATE_DIR;
    } else {
      delete process.env.OPENPAW_STATE_DIR;
    }

    if (originalEnv.OPENPAW_CONFIG_PATH !== undefined) {
      process.env.OPENPAW_CONFIG_PATH = originalEnv.OPENPAW_CONFIG_PATH;
    } else {
      delete process.env.OPENPAW_CONFIG_PATH;
    }

    if (originalEnv.OPENPAW_GATEWAY_PORT !== undefined) {
      process.env.OPENPAW_GATEWAY_PORT = originalEnv.OPENPAW_GATEWAY_PORT;
    } else {
      delete process.env.OPENPAW_GATEWAY_PORT;
    }

    if (originalEnv.OPENPAW_PROFILE !== undefined) {
      process.env.OPENPAW_PROFILE = originalEnv.OPENPAW_PROFILE;
    } else {
      delete process.env.OPENPAW_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENPAW_PROFILE: "dev",
        OPENPAW_STATE_DIR: "/tmp/openpaw-daemon-state",
        OPENPAW_CONFIG_PATH: "/tmp/openpaw-daemon-state/openpaw.json",
        OPENPAW_GATEWAY_PORT: "19001",
      },
