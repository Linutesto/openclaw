...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.openpaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OPENPAW_PROFILE = profile;

  const stateDir = env.OPENPAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.OPENPAW_STATE_DIR?.trim()) {
    env.OPENPAW_STATE_DIR = stateDir;
  }

  if (!env.OPENPAW_CONFIG_PATH?.trim()) {
    env.OPENPAW_CONFIG_PATH = path.join(stateDir, "openpaw.json");
  }

  if (profile === "dev" && !env.OPENPAW_GATEWAY_PORT?.trim()) {
    env.OPENPAW_GATEWAY_PORT = "19001";
  }
}
