...
import { resolveOpenPawPackageRoot } from "../infra/openpaw-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "openpaw doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "openpaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENPAW_REPO_URL = "https://github.com/openpaw/openpaw.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".openpaw");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openpaw@")) {
    return trimmed.slice("openpaw@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.OPENPAW_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENPAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENPAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENPAW_GIT_DIR to an empty folder or an openpaw checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENPAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveOpenPawPackageRoot({
  ...
  defaultRuntime.log(theme.heading("OpenPaw update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OpenPaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenPaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openpaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openpaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g openpaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g openpaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPENPAW_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPENPAW_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("openpaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openpaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("openpaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openpaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `openpaw update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENPAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENPAW_GIT_DIR to an empty folder or an openpaw checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OPENPAW_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenPaw to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openpaw update", "Update a source checkout (git)"],
        ["openpaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["openpaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["openpaw update --tag beta", "One-off update to a dist-tag or version"],
        ["openpaw update --no-restart", "Update without restarting the service"],
        ["openpaw update --json", "Output result as JSON"],
        ["openpaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openpaw update wizard", "Interactive update wizard"],
        ["openpaw --update", "Shorthand for openpaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openpaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openpaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openpaw update status", "Show channel + version status."],
          ["openpaw update status --json", "JSON output."],
          ["openpaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openpaw.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
