...
const execHostEnforced = process.env.OPENPAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPENPAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpenPawCliOnPath({ pathEnv: process.env.PATH ?? "" });
