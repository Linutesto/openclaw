...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openpaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".openpaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenPawConfig> = {},
): OpenPawConfig {
  const base: OpenPawConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "openpaw"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".openpaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
