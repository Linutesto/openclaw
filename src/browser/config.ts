import type { BrowserConfig, BrowserProfileConfig, OpenPawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENPAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENPAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openpaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENPAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENPAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenPaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openpaw/openpaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenPawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENPAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openpaw";
