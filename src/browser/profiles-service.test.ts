...
vi.mock("./chrome.js", () => ({
  resolveOpenPawUserDataDir: vi.fn(() => "/tmp/openpaw-test/openpaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOpenPawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "openpaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveOpenPawUserDataDir).mockReturnValue(userDataDir);
