...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const openpaw = resolveProfile(resolved, "openpaw");
    expect(openpaw?.driver).toBe("openpaw");
    expect(openpaw?.cdpPort).toBe(18800);
    expect(openpaw?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from OPENPAW_GATEWAY_PORT when unset", () => {
    const prev = process.env.OPENPAW_GATEWAY_PORT;
    process.env.OPENPAW_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const openpaw = resolveProfile(resolved, "openpaw");
      expect(openpaw?.cdpPort).toBe(19012);
      expect(openpaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.OPENPAW_GATEWAY_PORT;
      } else {
        process.env.OPENPAW_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const openpaw = resolveProfile(resolved, "openpaw");
      expect(openpaw?.cdpPort).toBe(19022);
      expect(openpaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.OPENPAW_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "openpaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        openpaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("openpaw");
  });
});
