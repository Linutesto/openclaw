  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-openpaw-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-openpaw-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-openpaw-labels", "1");
          ...
          tag.setAttribute("data-openpaw-labels", "1");
