  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "openpaw",
        profiles: {
          openpaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenPawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
    launchCalls.push({ port: profile.cdpPort });
    reachable = true;
    return {
      pid: 123,
      exe: { kind: "chrome", path: "/fake/chrome" },
      userDataDir: "/tmp/openpaw",
      cdpPort: profile.cdpPort,
      startedAt: Date.now(),
      proc,
    };
  }),
  resolveOpenPawUserDataDir: vi.fn(() => "/tmp/openpaw"),
...
describe("browser control server", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPENPAW_GATEWAY_PORT;
    process.env.OPENPAW_GATEWAY_PORT = String(testPort - 2);
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.OPENPAW_GATEWAY_PORT = prevGatewayPort;
    }
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (openpaw)
    expect(status.profile).toBe("openpaw");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("openpaw");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default openpaw profile
    expect(result.profiles.some((p) => p.name === "openpaw")).toBe(true);
  });

  it("GET /tabs?profile=openpaw returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=openpaw`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=openpaw opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=openpaw`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
