...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".openpaw"].map((dir) => path.resolve(root, entry.name, dir));
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".openpaw");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.openpaw out of git; it contains credentials and session history.",
