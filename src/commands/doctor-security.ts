...
import type { OpenPawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenPawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openpaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openpaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openpaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openpaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openpaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
