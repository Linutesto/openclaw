...
async function removeMacApp(runtime: RuntimeEnv, dryRun?: boolean) {
  ...
  await removePath("/Applications/OpenPaw.app", runtime, {
    dryRun,
    label: "/Applications/OpenPaw.app",
  });
}
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.openpaw" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/OpenPaw.app",
        },
      ],
