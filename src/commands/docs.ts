...
const SEARCH_TOOL = "https://docs.openpaw.ai/mcp.SearchOpenPaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openpaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openpaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openpaw.ai/");
      runtime.log(`Search: ${formatCliCommand('openpaw docs "your query"')}`);
    }
    return;
  }
