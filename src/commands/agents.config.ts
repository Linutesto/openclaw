...
export function listAgentEntries(cfg: OpenPawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenPawConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenPawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenPawConfig): AgentSummary[] {
