...
const requireAgentDir = () => {
  const agentDir = process.env.OPENPAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENPAW_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPENPAW_STATE_DIR;
  const previousAgentDir = process.env.OPENPAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OPENPAW_AGENT_DIR;
    } else {
      process.env.OPENPAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPENPAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENPAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-oauth-"));
    process.env.OPENPAW_STATE_DIR = tempStateDir;
    process.env.OPENPAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENPAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENPAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-minimax-"));
    process.env.OPENPAW_STATE_DIR = tempStateDir;
    process.env.OPENPAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENPAW_AGENT_DIR;
