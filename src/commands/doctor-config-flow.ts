...
function stripUnknownConfigKeys(config: OpenPawConfig): {
  config: OpenPawConfig;
  removed: string[];
} {
  const parsed = OpenPawSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: OpenPawConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".openpaw");
  const targetPath = path.join(targetDir, "openpaw.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openpaw doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("openpaw doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "openpaw doctor --fix" to remove these keys.');
    }
  }
