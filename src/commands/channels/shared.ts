...
import { type OpenPawConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export async function requireValidConfig(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("openpaw doctor")}.`);
    runtime.exit(1);
    return null;
  }
