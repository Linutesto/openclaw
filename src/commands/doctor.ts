...
function resolveMode(cfg: OpenPawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: OpenPawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("openpaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openpaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openpaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openpaw doctor --fix")}" to apply changes.`);
  }
