...
beforeAll(() => {
  previousProfile = process.env.OPENPAW_PROFILE;
  process.env.OPENPAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENPAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openpaw-root.js", () => ({
  resolveOpenPawPackageRoot: vi.fn().mockResolvedValue("/tmp/openpaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openpaw/pnpm-lock.yaml",
      markerPath: "/tmp/openpaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpenPaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openpaw status --all") ||
          l.includes("openpaw --profile isolated status --all") ||
          l.includes("openpaw status --all") ||
          l.includes("openpaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENPAW_GATEWAY_TOKEN = "abcd1234";
    try {
