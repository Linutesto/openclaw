...
beforeEach(() => {
  ...
  resolveOpenPawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENPAW_STATE_DIR;
  originalUpdateInProgress = process.env.OPENPAW_UPDATE_IN_PROGRESS;
  process.env.OPENPAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openpaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENPAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENPAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENPAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpenPawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenPawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/openpaw-root.js", () => ({
  resolveOpenPawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "openpaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENPAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENPAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openpaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENPAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
