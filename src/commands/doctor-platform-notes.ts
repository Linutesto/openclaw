...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".openpaw", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: OpenPawConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`OPENPAW_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
    (deps?.noteFn ?? note)(lines.join("\n"), "Gateway (macOS)");
  }

  const tokenEntries = [["OPENPAW_GATEWAY_TOKEN", await getenv("OPENPAW_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["OPENPAW_GATEWAY_PASSWORD", await getenv("OPENPAW_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "OPENPAW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use OPENPAW_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> OPENPAW_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
