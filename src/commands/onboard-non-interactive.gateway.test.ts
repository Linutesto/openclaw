...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.OPENPAW_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.OPENPAW_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPENPAW_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "openpaw");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPENPAW_STATE_DIR = stateDir;
    process.env.OPENPAW_CONFIG_PATH = path.join(stateDir, "openpaw.json");
