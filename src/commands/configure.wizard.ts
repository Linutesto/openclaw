    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from openpaw.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const baseConfig: OpenPawConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.openpaw.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("openpaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.OPENPAW_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.OPENPAW_GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.OPENPAW_GATEWAY_TOKEN;
        const password = nextConfig.gateway?.auth?.password ?? process.env.OPENPAW_GATEWAY_PASSWORD;
      ...
      while (true) {
        ...
        if (choice === "health") {
          ...
          const token = nextConfig.gateway?.auth?.token ?? process.env.OPENPAW_GATEWAY_TOKEN;
          const password =
            nextConfig.gateway?.auth?.password ?? process.env.OPENPAW_GATEWAY_PASSWORD;
          await waitForGatewayReachable({
            url: wsUrl,
            token,
            password,
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.OPENPAW_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.OPENPAW_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.OPENPAW_GATEWAY_TOKEN;
