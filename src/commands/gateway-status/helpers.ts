import type { OpenPawConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: OpenPawConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.OPENPAW_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.OPENPAW_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: OpenPawConfig) {
