...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENPAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENPAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENPAW_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENPAW_GATEWAY_PASSWORD");
  });
