...
export async function updateConfig(
  mutator: (cfg: OpenPawConfig) => OpenPawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenPawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenPawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openpaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
