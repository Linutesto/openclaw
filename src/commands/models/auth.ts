...
import { readConfigFileSnapshot, type OpenPawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpenPawConfig, model: string): OpenPawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("openpaw plugins install")}\`.`,
    );
  }
