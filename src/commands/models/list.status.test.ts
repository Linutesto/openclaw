...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenPawAgentDir: vi.fn().mockReturnValue("/tmp/openpaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openpaw-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenPawAgentDir: mocks.resolveOpenPawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenPawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openpaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openpaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/openpaw-agent-custom");
