...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenPawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openpaw models auth setup-token")}\` or \`${formatCliCommand("openpaw configure")}\`.`
          : `Run \`${formatCliCommand("openpaw configure")}\` or set an API key env var.`;
