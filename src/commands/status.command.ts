...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenPaw status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openpaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openpaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openpaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.openpaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openpaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openpaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openpaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openpaw gateway probe")}`);
  }
}
