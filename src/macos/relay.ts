...
declare const __OPENPAW_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENPAW_VERSION__ === "string" && __OPENPAW_VERSION__) ||
  process.env.OPENPAW_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureOpenPawCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[openpaw] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[openpaw] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
