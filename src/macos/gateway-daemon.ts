...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `openpaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OPENPAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OPENPAW_GATEWAY_BIND ??
