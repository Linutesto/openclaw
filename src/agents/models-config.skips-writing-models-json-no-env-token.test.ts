...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openpaw-models-" });
}

const MODELS_CONFIG: OpenPawConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureOpenPawModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureOpenPawModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureOpenPawModelsJson } = await import("./models-config.js");
      const { resolveOpenPawAgentDir } = await import("./agent-paths.js");

      await ensureOpenPawModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveOpenPawAgentDir(), "models.json");
