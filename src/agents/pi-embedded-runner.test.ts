...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-embedded-agent-"));
...
const ensureModels = (cfg: OpenPawConfig) => ensureOpenPawModelsJson(cfg, agentDir) as unknown;
