...
describe("createOpenPawCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createOpenPawCodingTools({
      config,
    ...
    const anthropicTools = createOpenPawCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createOpenPawCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createOpenPawCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
