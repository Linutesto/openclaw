...
type WebSearchConfig = NonNullable<OpenPawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenPawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("openpaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.openpaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://openpaw.ai",
      "X-Title": "OpenPaw Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
