...
function resolveExecConfig(cfg: OpenPawConfig | undefined) {
...
export function createOpenPawCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createOpenPawReadTool(freshReadTool)];
    }
