...
/**
 * Cleanup suspended OpenPaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenPawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-cli-images-"));
  const paths: string[] = [];
