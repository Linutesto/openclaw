  ...
  it("includes hardening and resource flags", () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "openpaw.sandbox=1",
        "--label",
        "openpaw.sessionKey=main",
        "--label",
        "openpaw.createdAtMs=1700000000000",
        "--label",
        "openpaw.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
        ...
        "--cap-drop",
        "ALL",
        // NOTE: no-new-privileges intentionally omitted (breaks exec on some hosts)
        "--security-opt",
        "seccomp=/tmp/seccomp.json",
        "--security-opt",
        "apparmor=openpaw-sandbox",
        "--dns",
        "1.1.1.1",
        "--add-host",
