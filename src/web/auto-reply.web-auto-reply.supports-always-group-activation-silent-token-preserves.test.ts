...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-session-"));
...
describe("web auto-reply", () => {
  ...
  it("supports always-on group activation with silent token and clears pending history", async () => {
    ...
    setLoadConfigMock(() => ({
      messages: {
        groupChat: { mentionPatterns: ["@openpaw"] },
      },
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "openpaw" explicitly).
    await capturedOnMessage?.({
      body: "openpaw ping",
      from: "123@g.us",
      conversationId: "123@g.us",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/openpaw-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/openpaw-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
