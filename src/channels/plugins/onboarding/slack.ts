...
function setSlackDmPolicy(cfg: OpenPawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenPaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenPaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: OpenPawConfig, allowFrom: string[]): OpenPawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
