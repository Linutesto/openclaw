...
function validateIdentityAvatar(config: OpenPawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpenPawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenPawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenPawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenPawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpenPawConfig)),
    ),
  };
}
