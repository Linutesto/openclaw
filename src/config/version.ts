...
export function parseOpenPawVersion(raw: string | null | undefined): OpenPawVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareOpenPawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseOpenPawVersion(a);
  const parsedB = parseOpenPawVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
