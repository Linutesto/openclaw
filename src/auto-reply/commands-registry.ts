...
export function isCommandEnabled(cfg: OpenPawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenPawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenPawConfig),
...
export function getCommandDetection(_cfg?: OpenPawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenPawConfig) {
