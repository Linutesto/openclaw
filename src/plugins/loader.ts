...
import { discoverOpenPawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenPawPluginDefinition;
  register?: OpenPawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenPawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenPawPluginDefinition;
...
export function loadOpenPawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpenPawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "openpaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpenPawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OpenPawPluginModule;
    } catch (err) {
