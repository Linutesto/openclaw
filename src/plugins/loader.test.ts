...
import { loadOpenPawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPENPAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openpaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openpaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENPAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpenPawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPENPAW_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadOpenPawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
