...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPENPAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENPAW_STATE_DIR = path.join(home, ".openpaw");
    ...
    await fs.mkdir(path.join(home, ".openpaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.openpaw${path.sep}media`);
