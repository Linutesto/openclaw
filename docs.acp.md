...
This document describes how the OpenPaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openpaw acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenPaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OpenPaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `openpaw acp` over stdio.

Example config:

```bash
openpaw config set gateway.remote.url wss://gateway-host:18789
openpaw config set gateway.remote.token <token>
```

Example run:

```bash
openpaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
openpaw acp --session agent:main:main
openpaw acp --session agent:design:main
openpaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OpenPaw ACP” to start a thread.

## Execution Model

- ACP client spawns `openpaw acp` and speaks ACP messages over stdio.
...
```bash
openpaw acp --session agent:main:main
openpaw acp --session-label "support inbox"
openpaw acp --reset-session
```
...
`openpaw acp` resolves the Gateway URL and auth from CLI flags or config:
