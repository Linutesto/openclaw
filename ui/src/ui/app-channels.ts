...
export async function handleWhatsAppStart(host: OpenPawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenPawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenPawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenPawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenPawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenPawApp): string {
...
export function handleNostrProfileCancel(host: OpenPawApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenPawApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: OpenPawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenPawApp) {
  const state = host.nostrProfileFormState;
