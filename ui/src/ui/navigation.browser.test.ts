...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openpaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openpaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openpaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENPAW_CONTROL_UI_BASE_PATH__ = "/openpaw";
    const app = mountApp("/openpaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openpaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openpaw/sessions");
  });
